"""Script to convert JSON dict values to strings in training data."""
...
def convert_dict_values_to_string(data: Any) -> Any:
    """Recursively convert dict values to JSON strings while preserving other types."""
    if isinstance(data, dict):
        # Convert the entire dict to a JSON string
        return json.dumps(data, ensure_ascii=False)
    elif isinstance(data, list):
        # Recursively process list items
        return [convert_dict_values_to_string(item) for item in data]
    else:
...
def process_training_data(input_file: Path, output_file: Path) -> None:
    """Process training data file to convert dict values to strings."""
    try:
    ...
    if isinstance(data, list):
        ...
        for entry in data:
            if isinstance(entry, dict) and "conversations" in entry:
                ...
                for conversation in entry["conversations"]:
                    ...
                    processed_conversation["value"] = convert_dict_values_to_string(
                        conversation["value"]
                    )
        ...
        processed_data = convert_dict_values_to_string(data)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Convert JSON dict values to strings in training data"
    )
    parser.add_argument(
        "-i",
        "--input",
